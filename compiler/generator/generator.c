...
typedef void(*generator)(block_t *block, node_t *node, struct variable *var);

static inline void gen_node(block_t *block, node_t *node, struct variable *var);

static void gen_unary_op(block_t *block, node_t *node, struct variable *var)
{
...
static void gen_binary_op(block_t *block, node_t *node, struct variable *var)
{
...
static void gen_num(block_t *block, node_t *node, struct variable *var)
{
	if (var)
		block_push(block, B_MOV_IMM, (rt_value)var, RT_INT2FIX(node->left), 0);
}

static void gen_var(block_t *block, node_t *node, struct variable *var)
{
	if(var)
	{
...
static void gen_string(block_t *block, node_t *node, struct variable *var)
{
	if (var)
		block_push(block, B_STRING, (rt_value)var, (rt_value)node->left, 0);
}

static void gen_string_continue(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_string_start(block_t *block, node_t *node, struct variable *var)
{
	struct variable *args = block_gen_args(block);
...
static void gen_const(block_t *block, node_t *node, struct variable *var)
{
	if (var)
	{
...
static void gen_self(block_t *block, node_t *node, struct variable *var)
{
	if (var)
	{
		block->self_ref++;
...
static void gen_true(block_t *block, node_t *node, struct variable *var)
{
	if (var)
		block_push(block, B_MOV_IMM, (rt_value)var, RT_TRUE, 0);
}

static void gen_false(block_t *block, node_t *node, struct variable *var)
{
	if (var)
		block_push(block, B_MOV_IMM, (rt_value)var, RT_FALSE, 0);
}

static void gen_nil(block_t *block, node_t *node, struct variable *var)
{
	if (var)
		block_push(block, B_MOV_IMM, (rt_value)var, RT_NIL, 0);
}

static void gen_assign(block_t *block, node_t *node, struct variable *var)
{
	struct variable *reading = (struct variable *)node->left;
...
static void gen_const_assign(block_t *block, node_t *node, struct variable *var)
{
	struct variable *self = block_get_var(block);
...
static void gen_if(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_expressions(block_t *block, node_t *node, struct variable *var)
{
...
static void gen_class(block_t *block, node_t *node, struct variable *var)
{
	block->self_ref++;
...
static void gen_module(block_t *block, node_t *node, struct variable *var)
{
	block->self_ref++;
...
static void gen_method(block_t *block, node_t *node, struct variable *var)
{
	block->self_ref++;
...
static int gen_argument(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static struct variable *get_upval(block_t *block, struct variable *var)
{
	if(var->real)
	{
		return var->real;
...
static void generate_call(block_t *block, node_t *self, rt_value name, node_t *arguments, node_t *block_node, struct variable *var)
{
...
static void gen_call(block_t *block, node_t *node, struct variable *var)
{
	generate_call(block, node->left, (rt_value)node->middle, node->right->left, node->right->right, var);
}

static void gen_array_call(block_t *block, node_t *node, struct variable *var)
{
	generate_call(block, node->left, rt_symbol_from_cstr("[]"), node->middle, node->right, var);
}

static size_t gen_array_element(block_t *block, node_t *node, struct variable *var)
{
...
static void gen_array(block_t *block, node_t *node, struct variable *var)
{
	struct variable *args = block_gen_args(block);
...
static void gen_boolean(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_not(block_t *block, node_t *node, struct variable *var)
{
	if(var)
	{
		struct variable *temp = var ? var : block_get_var(block);
...
static void gen_warn(block_t *block, node_t *node, struct variable *var)
{
	printf("node %d entered in code generation\n", node->type);
}

static void gen_ivar(block_t *block, node_t *node, struct variable *var)
{
	if(var)
	{
		block->self_ref++;
...
static void gen_ivar_assign(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_no_equality(block_t *block, node_t *node, struct variable *var)
{
	if(var)
	{
...
static void gen_return(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_break(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_next(block_t *block, node_t *node, struct variable *var)
{
	struct variable *temp = var ? var : block_get_var(block);
...
static void gen_redo(block_t *block, node_t *node, struct variable *var)
{
	block_push(block, B_REDO, 0, 0, 0);
}

static void gen_break_handler(block_t *block, node_t *node, struct variable *var)
{
...
static void gen_handler(block_t *block, node_t *node, struct variable *var)
{
