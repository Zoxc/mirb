...
#include "../runtime/classes/symbol.hpp"
	...
	void opcode_print(struct opcode *op)
	{
		switch(op->type)
		{
			...
			case B_ARGS:
				printf("args %d", op->result);
				break;

			case B_ARGS_POP:
				printf("args_pop 0x%x + %d", op->result, op->left);
				break;

			case B_CALL_ARGS:
				printf("call_args %d, %s", op->result, variable_name(op->left));
				break;

			case B_CALL_ARGS_POP:
				printf("call_args_pop 0x%x + %d", op->result, op->left);
				break;
